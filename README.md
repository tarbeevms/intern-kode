...
Сервис предоставляет API для входа, создания записки, редактирования, удаления и просмотра, соответственно по `POST /api/login`, `POST /api/note`, `PUT /api/note/{id}`, `DELETE /api/note/{id}`, `GET /api/note`.<br>
